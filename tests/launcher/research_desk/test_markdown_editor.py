...
def test_dirty_label_prefixes_bullet_when_dirty() -> None:
    assert is_dirty_label(path_name="RESEARCH.md", dirty=True) == "● RESEARCH.md"


def test_dirty_label_plain_when_clean() -> None:
    assert is_dirty_label(path_name="RESEARCH.md", dirty=False) == "RESEARCH.md"


def test_persona_path_prefers_agent_specific(tmp_path: Path) -> None:
    specific = tmp_path / "docs" / "agents" / "research.md"
    specific.parent.mkdir(parents=True)
    specific.write_text("# research", encoding="utf-8")

    (tmp_path / "AGENTS.md").write_text("# shared", encoding="utf-8")

    result = persona_path("RESEARCH", tmp_path)
...
def test_persona_path_returns_candidate_if_nothing_exists(tmp_path: Path) -> None:
    ...
    result = persona_path("REVIEW", tmp_path)
    expected = tmp_path / "docs" / "agents" / "review.md"
    assert result == expected
    assert not result.exists()


def test_persona_path_lowercases_key(tmp_path: Path) -> None:
    specific = tmp_path / "docs" / "agents" / "curate.md"
    specific.parent.mkdir(parents=True)
    specific.write_text("x", encoding="utf-8")
    result = persona_path("CURATE", tmp_path)
    assert result.name == "curate.md"


def test_persona_path_accepts_str_root(tmp_path: Path) -> None:
    result = persona_path("RESEARCH", str(tmp_path))
