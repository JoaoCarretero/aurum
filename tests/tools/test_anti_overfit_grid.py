...
def test_summarize_results_ranks_by_train_dsr_and_picks_conservative_variant():
    ...
    assert aggregate["best_train_variant"] == "BW00_baseline"
    assert aggregate["conservative_variant"] == "BW01_thresh_035"
...
def test_render_checklist_includes_auto_filled_sections():
    ...
    aggregate = {
        "train_rows": [
            {"variant": "BW00_baseline", "sharpe": 1.234, "sortino": 1.5, "max_dd_pct": 4.2, "n_trades": 130, "dsr": 0.96},
        ],
        "best_train_variant": "BW00_baseline",
        "best_train_sharpe": 1.234,
        "best_train_dsr": 0.96,
        "sharpe_std": 0.1,
        "dsr_passed": True,
        "top3_test_rows": [
            {"rank": 1, "variant": "BW00_baseline", "sharpe_train": 1.234, "sharpe_test": 1.05, "sortino_test": 1.2},
        ],
        "worst_top3_test_sharpe": 1.05,
        "test_passed": True,
        "test_pending": False,
        "conservative_variant": "BW00_baseline",
        "holdout_sharpe": 0.88,
        "holdout_passed": True,
    ...
    assert "# Engine Validation Checklist - BRIDGEWATER" in text
    assert "BW00_baseline" in text
...
def test_select_variants_can_target_explicit_names_in_grid_order():
    ...
    selected = select_variants(
        spec,
        variant_names=["BW05_cooldown_4", "BW01_thresh_035"],
    )
...
def test_select_variants_rejects_unknown_name():
    ...
    try:
        select_variants(spec, variant_names=["BW99_missing"])
    except ValueError as exc:
        assert "BW99_missing" in str(exc)
    else:
