...
def generate_report(all_trades, eq, mc, cond, ratios, mdd_pct, wf, wf_regime,
    ...
    # ── Institutional PNG charts (if generated by save_institutional_plots) ──
        ...
        if png_files:
            ...
            for p in png_files:
                ...
                imgs.append(
                    ...
                    f'<div style="color:{_GOLD};font-size:11px;letter-spacing:1px;'
                    f'margin-bottom:6px;">{title}</div>'
                    f'<img src="charts/{p.name}" style="width:100%;height:auto;'
                    f'border:1px solid {_BORDER};border-radius:4px;"/>'
                    f'</div>'
                )
            institutional_html = _section("Institutional Charts", "".join(imgs), "institutional-charts")
