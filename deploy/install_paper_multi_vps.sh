#!/usr/bin/env bash
# AURUM · installer pro template multi-instance paper runner do MILLENNIUM
# Uso:
#   bash deploy/install_paper_multi_vps.sh [/srv/aurum.finance] [aurum] [slot] [account_size]
set -euo pipefail
...
ENV_FILE="/etc/aurum/paper-${SLOT}.env"

echo "=== AURUM paper multi-instance installer ==="
echo "  repo:    ${REPO_PATH}"
...
AURUM_PAPER_ACCOUNT_SIZE=${ACCOUNT_SIZE}
AURUM_PAPER_LABEL=${SLOT}
EOF
echo "  OK"
...
echo "[5/6] enable + start millennium_paper@${SLOT}.service"
sudo systemctl enable "millennium_paper@${SLOT}.service"
sudo systemctl start "millennium_paper@${SLOT}.service"

sleep 10
echo "[6/6] status:"
sudo systemctl status "millennium_paper@${SLOT}.service" --no-pager -l | head -20

echo
echo "=== paper template instance ativa ==="
echo "  Logs:      sudo journalctl -u millennium_paper@${SLOT}.service -f"
echo "  Env file:  ${ENV_FILE}"
echo "  Stop:      sudo systemctl stop millennium_paper@${SLOT}.service"
