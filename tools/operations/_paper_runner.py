...
def _persist_trade_to_db(record: dict) -> None:
    """Persist a closed trade to SQLite live_trades — convenience companion
    to _append_jsonl(TRADES_PATH).

    JSONL stays the source of truth; DB is for cross-run SQL queries.
    Failures NEVER bubble — trade lifecycle must not depend on DB.
    See `tools/operations/millennium_paper.py:_persist_trade_to_db` for
    rationale (this is the per-engine sibling).
    """
    try:
        import sqlite3
        ...
        try:
            ...
            _log.getLogger(f"aurum.{ENGINE_NAME}.paper").warning(
