"""Pre-registered MEANREV snapback/rejection battery."""
...
DAYS = 180
GRID_DOC = "docs/engines/meanrev/2026-04-18_snapback_grid.md"
MIN_TRADES = 100
MIN_PF = 1.0
MIN_EXP_R = 0.0

VARIANTS: dict[str, dict] = {
    "SB00_wick_both": {"entry_mode": "wick_reclaim", "time_stop_bars": 48},
    "SB01_wick_both_strict": {
        "entry_mode": "wick_reclaim", "deviation_enter": 2.5,
        "reclaim_atr_min": 1.0, "time_stop_bars": 48,
    },
    "SB02_wick_short_only": {"entry_mode": "wick_reclaim", "time_stop_bars": 48, "side_filter": "short_only"},
    "SB03_wick_short_strict": {
        "entry_mode": "wick_reclaim", "deviation_enter": 2.5,
        "reclaim_atr_min": 1.0, "rsi_short_min": 75.0,
        "time_stop_bars": 48, "side_filter": "short_only",
    },
    "SB04_wick_short_stop25": {
        "entry_mode": "wick_reclaim", "atr_stop_mult": 2.5,
        "time_stop_bars": 48, "side_filter": "short_only",
    },
    "SB05_extreme_reclaim_both": {"entry_mode": "extreme_reclaim", "reclaim_deviation_exit": 1.0, "time_stop_bars": 48},
    "SB06_extreme_reclaim_short": {
        "entry_mode": "extreme_reclaim", "reclaim_deviation_exit": 0.5,
        "time_stop_bars": 48, "side_filter": "short_only",
    },
    "SB07_wick_both_scale2": {
        "entry_mode": "wick_reclaim", "time_stop_bars": 48,
        "scale_in_levels": 2, "scale_in_step_atr": 0.5,
    },
    "SB08_wick_short_scale2": {
        "entry_mode": "wick_reclaim", "time_stop_bars": 48, "side_filter": "short_only",
        "scale_in_levels": 2, "scale_in_step_atr": 0.5,
    },
    "SB09_wick_long_only": {"entry_mode": "wick_reclaim", "time_stop_bars": 48, "side_filter": "long_only"},
}


def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Run pre-registered MEANREV snapback grid.")
...
def main() -> None:
    ...
    out_root = Path(args.out) if args.out else Path("data/meanrev") / f"snapback_search_{ts}"
    ...
    raw_csv = out_root / "results_raw_order.csv"
    ...
    _write_markdown_report(summary, rows, {
        "timestamp": ts,
        "symbols": symbols,
        "days": args.days,
        "grid_doc": GRID_DOC,
