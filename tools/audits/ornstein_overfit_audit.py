"""Run analysis.overfit_audit on ORNSTEIN trade files (schema normalized)."""
...
def _normalize_ornstein_trade(trade: dict) -> dict:
    ...
    # ORNSTEIN uses regime_tag (e.g. "range/low_vol"); map the range/trend
    # part to a macro_bias-equivalent so overfit_audit test D can classify.
    regime_tag = str(trade.get("regime_tag", "") or "")
    if "trend" in regime_tag:
        macro_bias = "TREND"
    elif "range" in regime_tag:
        macro_bias = "RANGE"
    else:
        macro_bias = "NEUTRAL"
    ...
    return {
        ...
        "pnl": pnl,
        "result": result,
        "score": float(trade.get("omega_final", 0.0) or 0.0) / 100.0,
        "omega_final": trade.get("omega_final"),
...
def _load_and_normalize(path: Path) -> list[dict]:
    ...
    return [_normalize_ornstein_trade(t) for t in raw]
...
def main() -> int:
    ...
    ap.add_argument("trade_files", nargs="+", help="One or more ORNSTEIN trades.json files")
