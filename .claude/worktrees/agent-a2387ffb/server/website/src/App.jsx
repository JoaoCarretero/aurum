...
// SHARED COMPONENTS
// ═══════════════════════════════════════════════════════════
// Reveal on scroll. Bulletproof: (1) IntersectionObserver with rootMargin so
// the trigger fires BEFORE the element enters the viewport, (2) a safety
// timer that forces reveal regardless (so content NEVER gets stuck at
// opacity 0 if the observer misfires on rapid scroll / resize / mobile
// engines that throttle IO callbacks), (3) a noop path when IO is missing.
function Fade({ children, delay = 0, y = 20, as: Tag = "div" }) {
  const ref = useRef(null);
  const [v, setV] = useState(false);
  useEffect(() => {
    const el = ref.current;
    // No element or no IO support → reveal immediately.
    ...
    ob.observe(el);
    // Safety net: after 1.4s force reveal no matter what. Beats invisible
    // content if IO callback silently drops.
    const timer = setTimeout(() => { setV(true); ob.disconnect(); }, 1400);
    return () => { ob.disconnect(); clearTimeout(timer); };
  }, []);
  return (
    <Tag ref={ref} style={{
...
function Counter({ to, prefix = "", suffix = "", color = C.t, size = 32 }) {
  ...
  useEffect(() => {
    ...
    const rect = el.getBoundingClientRect();
    const vh = window.innerHeight || document.documentElement.clientHeight;
    if (rect.top < vh && rect.bottom > 0) { setStarted(true); return; }
    const ob = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) { setStarted(true); ob.disconnect(); }
    }, { threshold: 0, rootMargin: "0px 0px -40px 0px" });
    ob.observe(el);
    const t = setTimeout(() => { setStarted(true); ob.disconnect(); }, 1400);
  ...
  return (
    <div>
        ...
        <div className="container">
          <div className="hero-flex" style={{ display: "flex", alignItems: "center", gap: "clamp(40px, 6vw, 80px)" }}>
            <div style={{ flex: "1 1 420px" }}>
              ...
              <Fade delay={0.1}>
                <h1 style={{ fontSize: "clamp(38px, 5.8vw, 68px)", fontFamily: "var(--fd)", fontWeight: 400, lineHeight: 1.05, marginBottom: 28, letterSpacing: "-0.025em" }}>
                  {t ? "Capital gerido por " : "Capital managed by "}
                  <span style={{ fontStyle: "italic", color: C.gold, backgroundImage: `linear-gradient(120deg, ${C.gold2}, ${C.gold} 60%, #a88628)`, WebkitBackgroundClip: "text", backgroundClip: "text", WebkitTextFillColor: "transparent" }}>{t ? "inteligência quantitativa" : "quantitative intelligence"}</span>
                </h1>
              </Fade>
      ...
      <section ref={stratRef} className="section">
        <div className="container">
          <Fade>
            <div className="badge" style={{ marginBottom: 18 }}>{t ? "ESTRATEGIAS" : "STRATEGIES"}</div>
            <h2 style={{ fontSize: "clamp(30px, 3.8vw, 46px)", fontFamily: "var(--fd)", fontWeight: 400, marginBottom: 56, lineHeight: 1.1, letterSpacing: "-0.015em", maxWidth: 720 }}>
              {t ? "Tres engines, " : "Three engines, "}
              <span style={{ fontStyle: "italic", backgroundImage: `linear-gradient(120deg, ${C.gold2}, ${C.gold} 60%, #a88628)`, WebkitBackgroundClip: "text", backgroundClip: "text", WebkitTextFillColor: "transparent" }}>{t ? "zero correlacao" : "zero correlation"}</span>
            </h2>
          </Fade>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {engines.map((e, i) => (
              <Fade key={i} delay={i * 0.08}>
                <div className="glass strat-card" style={{ padding: "28px 30px", display: "flex", gap: 22, alignItems: "flex-start" }}>
                  <div className="strat-bar" style={{ width: 3, height: 52, borderRadius: 2, background: e.color, flexShrink: 0, boxShadow: `0 0 18px ${e.color}45` }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10, flexWrap: "wrap" }}>
                      <span className="mono" style={{ fontSize: 10, color: e.color, fontWeight: 600, background: `${e.color}12`, border: `1px solid ${e.color}22`, padding: "4px 10px", borderRadius: 4, letterSpacing: 1.2 }}>{e.id}</span>
                      <span style={{ fontSize: 15, fontWeight: 600, letterSpacing: "-0.01em" }}>{e.name}</span>
                    </div>
                    <p style={{ fontSize: 13.5, color: C.t2, lineHeight: 1.75, maxWidth: 720 }}>{e.desc}</p>
                  </div>
                </div>
              </Fade>
            ))}
      ...
      <section ref={perfRef} className="section">
        <div className="container">
          <Fade>
            <div className="badge" style={{ marginBottom: 18 }}>PERFORMANCE</div>
            <h2 style={{ fontSize: "clamp(30px, 3.8vw, 46px)", fontFamily: "var(--fd)", fontWeight: 400, marginBottom: 56, lineHeight: 1.1, letterSpacing: "-0.015em", maxWidth: 720 }}>
              {t ? "Resultados " : "Results "}
              <span style={{ fontStyle: "italic", backgroundImage: `linear-gradient(120deg, ${C.gold2}, ${C.gold} 60%, #a88628)`, WebkitBackgroundClip: "text", backgroundClip: "text", WebkitTextFillColor: "transparent" }}>{t ? "auditados" : "audited"}</span>
            </h2>
          </Fade>

          <Fade delay={0.1}>
            <div className="glass" style={{ padding: "28px 28px 20px", position: "relative", overflow: "hidden" }}>
              {/* Glow line effect */}
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 1, overflow: "hidden" }}>
                <div style={{ position: "absolute", width: "30%", height: "100%", background: `linear-gradient(to right, transparent, ${C.gold}60, transparent)`, animation: "glow-line 4s ease infinite" }} />
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(110px, 1fr))", gap: 0, marginBottom: 24, borderBottom: `1px solid ${C.brd}`, paddingBottom: 22 }}>
                {[
                  ...
                  <div key={i} style={{ textAlign: "center", padding: "2px 10px", borderRight: i < a.length - 1 ? `1px solid ${C.brd}` : "none" }}>
                    <div className="mono" style={{ fontSize: 20, fontWeight: 600, color: m.c, letterSpacing: "-0.02em" }}>{m.v}</div>
                    <div style={{ fontSize: 10, color: C.t3, marginTop: 6, letterSpacing: 1.5, textTransform: "uppercase", fontWeight: 500 }}>{m.l}</div>
                  </div>
                ))}
              </div>
      ...
      {/* ── FOOTER ─────────────────────────── */}
      <div className="section-line" />
      <footer style={{ padding: "56px 0 48px" }}>
        <div className="container" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <Logo size={22} />
            <span style={{ fontSize: 12, fontWeight: 600, letterSpacing: 4 }}>AURUM</span>
            <span style={{ fontSize: 10, color: C.t3, marginLeft: 10, fontFamily: "var(--fm)", letterSpacing: 1 }}>· QUANTITATIVE</span>
          </div>
          <div style={{ display: "flex", gap: 32, flexWrap: "wrap", justifyContent: "center" }}>
            {[t ? "Estrategias" : "Strategies", "Performance", "API", "Docs"].map((l, i) => (
              <span key={i} className="foot-link">{l}</span>
            ))}
          </div>
          <span style={{ fontSize: 10, color: C.t3, letterSpacing: 0.5 }}>&copy; 2026 AURUM Finance</span>
        </div>
      </footer>
    </div>
  );
...
export default function App() {
  ...
  return (
    <div style={{ background: C.bg, color: C.t, minHeight: "100vh", position: "relative" }}>
      ...
      {/* NAV */}
      <nav className={`aurum-nav${scrolled ? " scrolled" : ""}`}>
        <div className="nav-brand" onClick={() => { if (!user) setPage("land"); }}>
          <Logo size={24} />
          <span style={{ fontSize: 13, fontWeight: 600, letterSpacing: 4 }}>AURUM</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {user && <span style={{ fontSize: 11, color: C.t3, marginRight: 4, fontFamily: "var(--fm)" }}>{user.name || user.email}</span>}
          {user ? (
            <button onClick={handleLogout} className="btn-secondary" style={{ padding: "8px 18px", fontSize: 11 }}>{lang === "pt" ? "Sair" : "Logout"}</button>
          ) : page === "land" ? (
            <button onClick={() => setPage("auth")} className="btn-primary" style={{ padding: "9px 22px", fontSize: 11 }}>{lang === "pt" ? "Entrar" : "Sign In"}</button>
          ) : null}
          <button onClick={() => setLang(lang === "en" ? "pt" : "en")} className="nav-lang" aria-label="Toggle language">
            {lang === "en" ? "PT" : "EN"}
          </button>
        </div>
      </nav>
